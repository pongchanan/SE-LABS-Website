...
#     response = client.get("/user/news/thumbnail?amount=5")
#     assert response.status_code == 200
#     assert response.json() == fake_db_news[:5]

# def test_get_thumbnail_partial_events():
#     response = client.get("/user/news/thumbnail?amount=3")
#     assert response.status_code == 200
#     assert response.json() == fake_db_news[:3]

# def test_get_thumbnail_negative_amount():
#     with pytest.raises(HTTPException) as excinfo:
#         client.get("/user/news/thumbnail?amount=-1")
#     assert excinfo.value.status_code == 400
#     assert excinfo.value.detail == "Amount must be a positive integer"

# def test_get_thumbnail_zero_amount():
#     response = client.get("/user/news/thumbnail?amount=0")
#     assert response.status_code == 200
#     assert response.json() == []

# def test_get_thumbnail_too_much_amount():
#     with pytest.raises(HTTPException) as excinfo:
#         client.get("/user/news/thumbnail?amount=100")
#     assert excinfo.value.status_code == 400
#     assert excinfo.value.detail == "Amount exceeds the number of events in the database"

# def test_get_thumbnail_with_laboratory_id():
#     response = client.get("/user/news/thumbnail?amount=2&laboratory_id=1")
#     assert response.status_code == 200
...
#     response = client.get("/user/news/thumbnail?amount=2&research_id=1")
#     assert response.status_code == 200
#     assert len(response.json()) <= 2
#     for event in response.json():
...
#     response = client.get("/user/news/thumbnail?amount=2&laboratory_id=1&research_id=1")
#     assert response.status_code == 200
#     assert len(response.json()) <= 2
#     for event in response.json():
...
#     response = client.get("/user/news/thumbnail?amount=2&laboratory_id=999")
#     assert response.status_code == 200
#     assert response.json() == []

# def test_get_thumbnail_string_amount():
#     with pytest.raises(RequestValidationError) as excinfo:
#         client.get("/user/news/thumbnail?amount=abc")
...
#     with pytest.raises(RequestValidationError) as excinfo:
#         client.get("/user/news/thumbnail?amount=1.5")
#     exception = excinfo.value
#     errors = exception.errors()
#     assert len(errors) == 1
#     assert errors[0]['type'] == 'int_parsing'
...
#     with pytest.raises(RequestValidationError) as excinfo:
#         client.get("/user/news/thumbnail")
#     exception = excinfo.value
#     errors = exception.errors()
#     assert len(errors) == 1
